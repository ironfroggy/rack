            ...
            with error_recovery(debug=debug) as recover:
                ...
                if line_status.lower().startswith("bin"):
                    ...
                    try:
                        ...
                        if not recover(f"Error parsing diff stat line: {line!r}", e):
...
parser.add_argument("-u", "--show-untracked",
...
list_parser.add_argument("-u", "--show-untracked",
    help="show untracked files",
    action="store_true",
)
list_parser.add_argument("-R", "--show-remotes",
    help="show remotes",
    action="store_true",
)
list_parser.add_argument("-N", "--non-repos",
    help="show non-repos",
    action="store_true",
    dest="non_repos",
)
list_parser.add_argument("-O", "--only-non-repos",
...
def main(argv=None):
    ...
    if ns.changed:
        repos = [
            repo
            for repo in repos
            if git_status_files(os.path.join(ns.working_dir, repo), ns.show_untracked, debug=ns.debug)
        ]
    ...
    match ns.action:
        case "list":
            if ns.non_repos_only and not ns.non_repos:
                logger.error("Cannot specify -O/--only-non-repos without -N/--non-repos")
                return 1
            ...
            for i, directory in enumerate(directories):
                ...
                if ns.status:
                    ...
                    files = git_status_files(directory_path, ns.show_untracked, debug=ns.debug)
                    changes = len(files)
